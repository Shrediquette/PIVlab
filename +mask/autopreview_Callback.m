...
if size(filepath,1) > 1 %did the user load images?
	...
	[~,piv_image_A]=import.import_get_img(selected);%[piv_image_A,~]=get_img(selected); would respect background subtraction
	...
	image(cat(3, piv_image, piv_image, piv_image), 'parent',gui.gui_retr('pivlab_axis'), 'cdatamapping', 'scaled');
	hold on;
	colormap('gray');
	axis image
	set(gui.gui_retr('pivlab_axis'),'ytick',[])
	set(gui.gui_retr('pivlab_axis'),'xtick',[])
	image(cat(3, pixel_mask*0.7, pixel_mask*0.1, pixel_mask*0.1), 'parent',gui.gui_retr('pivlab_axis'), 'cdatamapping', 'direct','AlphaData',pixel_mask*0.9);
	hold off
end
