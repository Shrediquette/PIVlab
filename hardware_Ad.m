...
handles.text0 = uicontrol(fig_handle,'Style','text','units', 'characters','Fontweight','bold', 'Fontunits','points','Fontsize',14,'Horizontalalignment', 'left','position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'String','Did you know?');
...
handles.vidsi = uicontrol(fig_handle,'Style','pushbutton','String','Click to watch Youtube video','CData',bgim,'Units','characters','Fontweight','bold', 'Fontunits','points','Fontsize',12,'Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'TooltipString','Opens William''s video on YOUTUBE','Callback',@gotovideo);
...
imshow(imread('hardware_Ad.jpg'));drawnow
	...
	function gotovideo(~,~)
		web('https://www.youtube.com/watch?v=8IQSODoYUr0')
		pause(1)
		close(fig_handle)
	end
