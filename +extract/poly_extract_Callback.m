...
if (gui.retr('calu')==1 || gui.retr('calu')==-1) && gui.retr('calxy')==1
	set(handles.extraction_choice,'string', {'Vorticity in 1/frame';'Magnitude in px/frame';'u component in px/frame';'v component in px/frame';'Divergence in 1/frame';'Q criterion in 1/frame^2';'Shear rate in 1/frame';'Strain rate in 1/frame';'Vector direction in degrees';'Correlation coefficient';'Uncertainty in px/frame';'Tangent velocity in px/frame'});
else %calibrated
	displacement_only=gui.retr('displacement_only');
	if ~isempty(displacement_only) && displacement_only == 1
		set(handles.extraction_choice,'string', {'Vorticity in 1/frame';'Magnitude in m/frame';'u component in m/frame';'v component in m/frame';'Divergence in 1/frame';'Q criterion in 1/frame^2';'Shear rate in 1/frame';'Strain rate in 1/frame';'Vector direction in degrees';'Correlation coefficient';'Uncertainty in m/frame';'Tangent velocity in m/frame'});
	else
		set(handles.extraction_choice,'string', {'Vorticity in 1/s';'Magnitude in m/s';'u component in m/s';'v component in m/s';'Divergence in 1/s';'Q criterion in 1/s^2';'Shear rate in 1/s';'Strain rate in 1/s';'Vector direction in degrees';'Correlation coefficient';'Uncertainty in m/s';'Tangent velocity in m/s'});
	end
end
