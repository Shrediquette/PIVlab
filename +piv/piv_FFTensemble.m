...
image1 = preproc.PIVlab_preproc( ...
    in=image1, roirect=roi_inpt, clahe=clahe, clahesize=clahesize, ...
    highp=highp, highpsize=highpsize, intenscap=intenscap, ...
    wienerwurst=wienerwurst, wienerwurstsize=wienerwurstsize, ...
    minintens=minintens1, maxintens=maxintens1);
image2 = preproc.PIVlab_preproc( ...
    in=image2, roirect=roi_inpt, clahe=clahe, clahesize=clahesize, ...
    highp=highp, highpsize=highpsize, intenscap=intenscap, ...
    wienerwurst=wienerwurst, wienerwurstsize=wienerwurstsize, ...
...
[utable,vtable] = postproc.PIVlab_postproc( ...
    u=utable, v=vtable, valid_vel=[], do_stdev_check=1, stdthresh=4, ...
    do_local_median=1, neigh_thresh=1.5);
            if GUI_avail==1
                cancel=getappdata(hgui, 'cancel');
                if cancel == 1
                    break
