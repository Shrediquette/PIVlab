...
if numel(filepath)>1 && get(handles.algorithm_selection,'Value') ~= 4 %display only when files are loaded and not using wOFV
	...
	if get(handles.algorithm_selection ,'value')==1 || get(handles.algorithm_selection ,'value')==2
		...
		if get(handles.checkbox28,'value')==1
			...
			text(target_axis,x4(1),y4(3), ['pass 4'],'color','r','fontsize',8,'tag','intareadispl','HorizontalAlignment','right','verticalalignment','top')
		end
	end
	hold(target_axis,'off')
	%check if step is ok
	if step/(intarea1*2) < 0.25
		text (target_axis,centre(2),centre(1)/2,'Warning: Step of pass 1 is very small.','color','r','tag','intareadispl','HorizontalAlignment','center','verticalalignment','top','Fontsize',10,'Backgroundcolor','k')
	end
