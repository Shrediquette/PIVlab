		...
		if currentwasmean==0
			...
			if get(handles.algorithm_selection,'Value')==1  %fft
				...
				if get(handles.algorithm_selection,'Value')==1 %fft multi
					try
						[x, ~, ~, ~, ~,~,correlation_matrices,all_xy_tables] = piv.piv_FFTmulti (image1,image2,interrogationarea, step, subpixfinder, converted_mask, roirect,passes,int2,int3,int4,imdeform,repeat,mask_auto,do_pad,do_correlation_matrices,repeat_last_pass,delta_diff_min);
					catch ME
						disp(getReport(ME))
