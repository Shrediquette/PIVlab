...
if size(event.Modifier,2)==2 && strcmp(event.Modifier{1},'shift') && strcmp(event.Modifier{2},'control') %ctrl and shift modifiers
	if strcmp(event.Key,'c')
		crosshair_enabled=gui_NameSpace.gui_retr('crosshair_enabled');
		if isempty(crosshair_enabled)
			crosshair_enabled=0;
		end
		gui_NameSpace.gui_put('crosshair_enabled',1-crosshair_enabled);
	elseif strcmp(event.Key,'x')
		sharpness_enabled=gui_NameSpace.gui_retr('sharpness_enabled');
		if isempty(sharpness_enabled)
			sharpness_enabled=0;
		end
		gui_NameSpace.gui_put('sharpness_enabled',1-sharpness_enabled); % only autofocs OR sharpness display must be enabled at a time
	elseif strcmp(event.Key,'hyphen') %minus key
		ac_upper_clim = gui_NameSpace.gui_retr('ac_upper_clim');
		if ac_upper_clim < 2^16
			ac_upper_clim = ac_upper_clim + 5000;
		end
		gui_NameSpace.gui_put('ac_upper_clim',ac_upper_clim);
		gui_NameSpace.gui_put('ac_lower_clim',0);
	...
	elseif strcmp(event.Key,'h') %
		hist_enabled=gui_NameSpace.gui_retr('hist_enabled');
		if isempty(hist_enabled)
			hist_enabled=0;
		end
		gui_NameSpace.gui_put('hist_enabled',1-hist_enabled);
	end
end
