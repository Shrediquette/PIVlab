...
[x_cal,y_cal]=calibrate_NameSpace.calibrate_xy (x,y);
...
if typevector(info(1,1),info(1,2)) ~=0
	...
	if (gui_NameSpace.gui_retr('calu')==1 || gui_NameSpace.gui_retr('calu')==-1) && gui_NameSpace.gui_retr('calxy')==1%not calibrated
		set(handles.u_cp, 'String', ['u:' num2str(round((u(info(1,1),info(1,2))*gui_NameSpace.gui_retr('calu')-gui_NameSpace.gui_retr('subtr_u'))*100000)/100000) ' px/fr']);
		set(handles.v_cp, 'String', ['v:' num2str(round((v(info(1,1),info(1,2))*gui_NameSpace.gui_retr('calv')-gui_NameSpace.gui_retr('subtr_v'))*100000)/100000) ' px/fr']);
		...
		u_cp_string_velocity=u(info(1,1),info(1,2))*gui_NameSpace.gui_retr('calu')-gui_NameSpace.gui_retr('subtr_u');
		v_cp_string_velocity=v(info(1,1),info(1,2))*gui_NameSpace.gui_retr('calv')-gui_NameSpace.gui_retr('subtr_v');
		...
		if size (derived,2) >= (currentframe+1)/2
			if numel(derived{displaywhat-1,(currentframe+1)/2})>0
				...
				try
					...
					plot_NameSpace.plot_derivs_Callback
					name=get(handles.derivchoice,'string');
					scalar_to_plot=map(info(1,1),info(1,2));
					if scalar_to_plot >100 || scalar_to_plot < 0.001
						scalar_to_plot_string = sprintf('%0.4e',scalar_to_plot);
