...
if get(handles.bg_subtract,'Value')==1
	if mod(selected,2)==1 %uneven image nr.
		bg_img = gui_NameSpace.gui_retr('bg_img_A');
	else
		bg_img = gui_NameSpace.gui_retr('bg_img_B');
	end
...
if isempty(gui_NameSpace.gui_retr('size_warning_has_been_shown'))
	gui_NameSpace.gui_put('size_warning_has_been_shown',0);
end
if isempty(expected_image_size) %expected_image_size is empty, we have not read an image before
	...
	gui_NameSpace.gui_put('expected_image_size',expected_image_size);
else %expected_image_size is not empty, an image has been read before
	if 	(expected_image_size(1) ~= size_of_the_image(1) || expected_image_size(2) ~= size_of_the_image(2)) && gui_NameSpace.gui_retr('size_warning_has_been_shown') == 0
		piv_NameSpace.piv_cancelbutt_Callback
		uiwait(warndlg('Error: All images in a session  MUST have the same size!'));
		gui_NameSpace.gui_put('size_warning_has_been_shown',1);
		warning off
		recycle('off');
		delete('cancel_piv');
		warning on
...
gui_NameSpace.gui_put('size_of_the_image',size_of_the_image);
