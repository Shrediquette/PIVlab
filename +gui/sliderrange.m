...
if gui.retr('video_selection_done') == 0
    if size(filepath,1)>2
        ...
        amount_frames_per_large_step= ceil((sliderstepcount *0.1));
        slidersteps=[1/(sliderstepcount-1) 1/(sliderstepcount-1)*amount_frames_per_large_step]; % sets steps to 1 frame (click on arrow) or 10% of whole range (click on trough)
