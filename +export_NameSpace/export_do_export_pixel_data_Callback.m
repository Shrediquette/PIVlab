...
imgsavepath=gui_NameSpace.gui_retr('imgsavepath');
if isempty(imgsavepath)
	imgsavepath=gui_NameSpace.gui_retr('pathname');
end
...
if ~isequal(filename,0) && ~isequal(pathname,0)
	gui_NameSpace.gui_put('imgsavepath',pathname );
	...
	gui_NameSpace.gui_put('export_axis',export_axis);
	pause(0.01)
	try
		...
		for i=startframe:endframe
			...
			gui_NameSpace.gui_sliderdisp(export_axis)
			...
			switch selected_format
				case 'PNG'
					if use_exportfig
						...
						export_NameSpace.export_autocrop(fullfile(pathname,newfilename),0);
					else
				...
				case 'JPG'
					if use_exportfig
						exportfig(export_figure,fullfile(pathname,newfilename),'Format','bmp','color','rgb','linemode','scaled','FontMode','scaled','FontSizeMin',16,'Bounds','loose','resolution',resolution)
						export_NameSpace.export_autocrop(fullfile(pathname,newfilename),1);
					else
						exportgraphics(export_axis,fullfile(pathname,newfilename),'ContentType','image','resolution',resolution);
					end
				case 'PDF'
				...
				case 'Archival AVI'
					...
					if i==startframe %this makes sure that frame sizes keep the same size
						[export_image,croprect] = export_NameSpace.export_autocrop(export_image,3);
					else
	...
	gui_NameSpace.gui_sliderdisp(pivlab_axis)
end
