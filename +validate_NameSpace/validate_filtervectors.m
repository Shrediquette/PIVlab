...
if size(resultslist,2)>=frame
	...
	if size(manualdeletion,2)>=frame
		if isempty(manualdeletion{1,frame}) ==0
			...
			[u,v,typevector]=validate_NameSpace.validate_manual_point_deletion(u,v,typevector,framemanualdeletion);
		end
	end
	if numel(u)>0
		...
		if do_contrast_filter == 1 || do_bright_filter == 1
			...
			[A,rawimageA]=import_NameSpace.import_get_img(selected);
			[B,rawimageB]=import_NameSpace.import_get_img(selected+1);
