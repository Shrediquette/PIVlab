...
if ~isempty(resultslist)
	...
	if gui_NameSpace.gui_retr('video_selection_done') == 1 || gui_NameSpace.gui_retr('parallel')==0 %if post-processing a video, parallelization cannot be used.
		for i=1:num_frames_to_process
			validate_NameSpace.validate_filtervectors(i)
		...
		manualdeletion=gui_NameSpace.gui_retr('manualdeletion');
			...
			for i=1:size(u,2)
				if size(manualdeletion,2)>=i
					if isempty(manualdeletion{1,i}) ==0
						...
						[u{i},v{i},typevector{i}]=validate_NameSpace.validate_manual_point_deletion(u{i},v{i},typevector{i},framemanualdeletion);
					end
				end
			end
		end
		velrect=gui_NameSpace.gui_retr('velrect');
