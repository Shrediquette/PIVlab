...
if size(resultslist,2)>=frame && numel(resultslist{1,frame})>0 %analysis exists
	filenames=gui_NameSpace.gui_retr('filenames');
	...
	[currentimage,~]=import_NameSpace.import_get_img(2*frame-1);
	...
	if get(handles.interpol_missing,'value')==1
		if any(any(isnan(u))) || any(any(isnan(v)))
			if isempty(strfind(get(handles.apply_deriv_all,'string'), 'Please'))==1 && isempty(strfind(get(handles.ascii_all,'string'), 'Please'))==1 && isempty(strfind(get(handles.save_mat_all,'string'), 'Please'))==1%not in batch
				drawnow;
				if gui_NameSpace.gui_retr('alreadydisplayed') == 1
				else
				...
				gui_NameSpace.gui_put('alreadydisplayed',1);
			end
	...
	if deriv==7
		derived{6,frame}=plot_NameSpace.plot_dcev(x_adjusted*calxy,y_adjusted*calxy,u*calu,v*calv);
		%disp('dcev')
	end
	if deriv==8
		derived{7,frame}=plot_NameSpace.plot_shear(x_adjusted*calxy,y_adjusted*calxy,u*calu,v*calv);
		%disp('shear')
	end
	if deriv==9
		derived{8,frame}=plot_NameSpace.plot_strain(x_adjusted*calxy,y_adjusted*calxy,u*calu,v*calv);
		%disp('strain')
	end
	if deriv==10
		%{
		...
		%Original:
		derived{9,frame}=plot_NameSpace.plot_LIC(v*calv-subtr_v,u*calu-subtr_u,frame);
		%disp('LIC')
	end
	if deriv==11
		try
	...
	gui_NameSpace.gui_put('subtr_u', subtr_u);
	gui_NameSpace.gui_put('subtr_v', subtr_v);
	gui_NameSpace.gui_put('resultslist', resultslist);
	gui_NameSpace.gui_put ('derived',derived);
	if update==1
		gui_NameSpace.gui_put('displaywhat', deriv);
	end
end
