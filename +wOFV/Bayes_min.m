function [func,gd]=Bayes_min(Theta,I0,I1,scale,eta,vartheta,Fw,FwInv,Ni,...
...
v=cat(3,wOFV.Psitrunc_mat(Theta(:,:,1),FwInv),wOFV.Psitrunc_mat(Theta(:,:,2),FwInv));
        
%Motion-compensated images
IwF = wOFV.warpS(I0,v/2,Grid);
IwR=wOFV.warpS(I1,-v/2,Grid);
...
gd=cat(3,wOFV.Psitrunc_mat(gd(:,:,1),Fw),wOFV.Psitrunc_mat(gd(:,:,2),Fw));

%Compute regularization gradient and functional
gdr=wOFV.Reg(Theta,Ni,regscheme);
fr=.5*Theta(:)'*gdr;
