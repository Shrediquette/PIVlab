	...
	if ~isempty(xposition)
		...
		switch extractwhat
			...
			case 11 %tangent
				if ~strcmp(extract_type,'extract_circle_series')
					...
					u=u*calu-gui_NameSpace.gui_retr('subtr_u');
					v=v*calv-gui_NameSpace.gui_retr('subtr_v');

					u=plot_NameSpace.plot_rescale_maps_nan(u,0,currentframe);
					v=plot_NameSpace.plot_rescale_maps_nan(v,0,currentframe);
		...
		if ~strcmp(extract_type,'extract_circle_series') %user did not choose circle series
			...
			if strcmp (A.Tag,'plot_data') %function called from button press
				...
				gui_NameSpace.gui_put ('h_extractionplot', h_extractionplot);
				xlabel(['Distance on line' distunit_2 sprintf('\n') 'Integral of ' currentstripped ' = ' num2str(integral) ' [' unitpar '*' distunit_2 ']']);
				ylabel(current);
			end
			%modified units...
			gui_NameSpace.gui_put('distance',distance*calxy);
			gui_NameSpace.gui_put('c',c);
			[cx_cal,cy_cal] = calibrate_NameSpace.calibrate_xy(cx,cy);
			gui_NameSpace.gui_put('cx',cx_cal);
			gui_NameSpace.gui_put('cy',cy_cal);
		end
		if strcmp(extract_type,'extract_circle_series') %user chose circle series
			calxy=gui_NameSpace.gui_retr('calxy');
			...
			if ~isempty(radii(col))
				extract_poly_maximum_circ=drawcircle(gui_NameSpace.gui_retr('pivlab_axis'),'Center',xposition,'Radius',radii(col),'Tag',[extract_type '_max_circulation'],'Deletable',0,'FaceAlpha',0,'FaceSelectable',0,'InteractionsAllowed','none','Color','r','StripeColor','y');
				current=get(handles.extraction_choice,'string');
				current=current{extractwhat};
				calxy=gui_NameSpace.gui_retr('calxy');
				if strcmp (A.Tag,'plot_data') %function called from button press
				...
				gui_NameSpace.gui_put('cx',cx_cal);
				gui_NameSpace.gui_put('cy',cy_cal);
				gui_NameSpace.gui_put('integral', integral);
			end
		end
	end
end
