...
for i=startfr:endfr
	...
	if size(resultslist,2)>=currentframe && numel(resultslist{1,currentframe})>0
		...
		caller.Tag=i;
		extract_NameSpace.extract_plot_data_Callback(caller) %make sure that data was calculated
		%close figure...
		%delete(findobj('tag', 'derivplotwindow'));
		...
		if selected==0
			...
			selected=1;
			gui_NameSpace.gui_toolsavailable(0,'Busy, extracting data...');drawnow
		end
		if isequal(FileName,0) | isequal(PathName,0)
			...
			c=gui_NameSpace.gui_retr('c');
			distance=gui_NameSpace.gui_retr('distance');
			%also retrieve coordinates of polyline points if possible
			cx=gui_NameSpace.gui_retr('cx');
			cy=gui_NameSpace.gui_retr('cy');
			%normal : 300 x 1
