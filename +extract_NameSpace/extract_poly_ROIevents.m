...
switch(evname)
	...
	case{'ROIMoved'}
		if strcmp(src.Tag,'extract_poly') || strcmp(src.Tag,'extract_poly_area')
			gui_NameSpace.gui_put('xposition',src.Position(:,1));
			gui_NameSpace.gui_put('yposition',src.Position(:,2));
		end
		if strcmp(src.Tag,'extract_circle') || strcmp(src.Tag,'extract_circle_area')
			gui_NameSpace.gui_put('xposition',src.Center);
			gui_NameSpace.gui_put('yposition',src.Radius);
		end
		if strcmp(src.Tag,'extract_rectangle_area')
			gui_NameSpace.gui_put('xposition',[src.Position(1) src.Position(3)]); %x and width of rectangle
			gui_NameSpace.gui_put('yposition',[src.Position(2) src.Position(4)]); %y and height of rectangle
		end
		if strcmp(src.Tag,'extract_circle_series') ||strcmp(src.Tag,'extract_circle_series_area')
			delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag',[src.Tag '_displayed_smaller_radii']))
			delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag',[src.Tag '_max_circulation']))
			...
			for radius=radii
				extract_poly_series=drawcircle(gui_NameSpace.gui_retr('pivlab_axis'),'Center',src.Center,'Radius',radius,'Tag',[src.Tag '_displayed_smaller_radii'],'Deletable',0,'FaceAlpha',0,'FaceSelectable',0,'InteractionsAllowed','none');
			end
	...
	case{'DeletingROI'}
		...
		delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag',src.Tag))
