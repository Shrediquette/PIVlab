...
if strcmp(extract_type,'extract_poly') %polyline
	extract_poly=drawpolyline(gui_NameSpace.gui_retr('pivlab_axis'),'Position',[xposition yposition]);
...
if strcmp(extract_type,'extract_poly_area') %polygon
	extract_poly=drawpolygon(gui_NameSpace.gui_retr('pivlab_axis'),'Position',[xposition yposition]);
	extract_poly.LabelVisible = 'off';
	extract_poly.Tag=extract_type;
	addlistener(extract_poly,'ROIMoved',@extract_NameSpace.extract_poly_ROIevents);
	addlistener(extract_poly,'MovingROI',@extract_NameSpace.extract_poly_ROIevents);
...
if strcmp(extract_type,'extract_circle') || strcmp(extract_type,'extract_circle_area') %circle
	extract_poly=drawcircle(gui_NameSpace.gui_retr('pivlab_axis'),'Center',xposition,'Radius',yposition);
	extract_poly.LabelVisible = 'off';
	extract_poly.Tag=extract_type;
	addlistener(extract_poly,'ROIMoved',@extract_NameSpace.extract_poly_ROIevents);
	addlistener(extract_poly,'DeletingROI',@extract_NameSpace.extract_poly_ROIevents);
...
if strcmp(extract_type,'extract_rectangle_area')  %rectangle
	extract_poly=drawrectangle(gui_NameSpace.gui_retr('pivlab_axis'),'Position',[xposition(1) yposition(1) xposition(2) yposition(2)]);
	extract_poly.LabelVisible = 'off';
	extract_poly.Tag=extract_type;
	addlistener(extract_poly,'ROIMoved',@extract_NameSpace.extract_poly_ROIevents);
	addlistener(extract_poly,'DeletingROI',@extract_NameSpace.extract_poly_ROIevents);
