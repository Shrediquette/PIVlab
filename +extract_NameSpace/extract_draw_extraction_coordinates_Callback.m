...
gui_NameSpace.gui_put('extract_type',[]);
if size(resultslist,2)>=currentframe && numel(resultslist{1,currentframe})>0
	gui_NameSpace.gui_toolsavailable(0);
	xposition=[];
	yposition=[];
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_poly')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_series')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_poly_area')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_area')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_series_area')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_rectangle_area')); %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_series_displayed_smaller_radii')) %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_series_area_displayed_smaller_radii')) %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_series_max_circulation')) %delete pre-existing
	delete(findobj(gui_NameSpace.gui_retr('pivlab_axis'),'Tag','extract_circle_series_area_max_circulation')) %delete pre-existing
	...
	if (strcmp(caller.Tag,'draw_stuff') && strcmp(handles.draw_what.String{handles.draw_what.Value},'polyline')) || (strcmp(caller.Tag,'draw_stuff_area') && strcmp(handles.draw_what_area.String{handles.draw_what_area.Value},'polygon'))%polyline
		...
		draw(extract_poly);
		addlistener(extract_poly,'ROIMoved',@extract_NameSpace.extract_poly_ROIevents);
		addlistener(extract_poly,'MovingROI',@extract_NameSpace.extract_poly_ROIevents);
		addlistener(extract_poly,'DeletingROI',@extract_NameSpace.extract_poly_ROIevents);
	...
	elseif (strcmp(caller.Tag,'draw_stuff') && strcmp(handles.draw_what.String{handles.draw_what.Value},'circle series (tangent vel. only)')) || (strcmp(caller.Tag,'draw_stuff_area') && strcmp(handles.draw_what_area.String{handles.draw_what_area.Value},'circle series'))%circle series
		...
		for radius=radii
			extract_poly_series=drawcircle(gui_NameSpace.gui_retr('pivlab_axis'),'Center',xposition,'Radius',radius,'Tag',[extract_type '_displayed_smaller_radii'],'Deletable',0,'FaceAlpha',0,'FaceSelectable',0,'InteractionsAllowed','none');
		end
