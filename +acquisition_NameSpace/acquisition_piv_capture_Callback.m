...
if required_files_check
	...
	if strmatch(button,'Yes')==1
		...
		if get(handles.ac_pivcapture_save,'Value')==1 %check settings only when user wants to save data
			...
			capture_ok=acquisition_NameSpace.acquisition_check_project_path(projectpath,'double_images');
		else
		...
		ac_ROI_realtime=gui_NameSpace.gui_retr('ac_ROI_realtime');
		do_realtime=gui_NameSpace.gui_retr('do_realtime');
		...
		if capture_ok==1
			...
			if isinf(imageamount)
				gui_NameSpace.gui_toolsavailable(0,'Starting PIV preview...')
			else
				gui_NameSpace.gui_toolsavailable(0,'Starting PIV capture...')
			end
			...
			if ~isinf(imageamount)
				...
				OPTOcam_bits =gui_NameSpace.gui_retr('OPTOcam_bits');
			...
			if (~isempty(gui_NameSpace.gui_retr('ac_enable_seeding1')) && gui_NameSpace.gui_retr('ac_enable_seeding1') ~=0) || (~isempty(gui_NameSpace.gui_retr('ac_enable_device1')) && gui_NameSpace.gui_retr('ac_enable_device1') ~=0) || (~isempty(gui_NameSpace.gui_retr('ac_enable_device2')) && gui_NameSpace.gui_retr('ac_enable_device2') ~=0) || (~isempty(gui_NameSpace.gui_retr('ac_enable_flowlab')) && gui_NameSpace.gui_retr('ac_enable_flowlab') ~=0)
				acquisition_NameSpace.acquisition_external_device_control(1); %starts activated devices
				waitbar(.15,f,'Starting external devices...');
				pause(0.5)
				waitbar(.33,f,'Starting external devices...');
				pause(0.5)
				if (~isempty(gui_NameSpace.gui_retr('ac_enable_flowlab')) && gui_NameSpace.gui_retr('ac_enable_flowlab') ~=0) %flowlab is activated
					%ask if flowlab was already running
					%if yes --> proceed, if not --> pause to wait for uniform flow velocity.
					flowlab_percent=gui_NameSpace.gui_retr('flowlab_percent');
					if flowlab_percent ~= 0
						acquisition_NameSpace.acquisition_external_device_control(1);
			...
			if value==1 || value==2 %setup withOUT LD-PS
				...
				gui_NameSpace.gui_put('laser_running',1);
				pause(1)
			...
			if value== 3 || value == 4 %setup with LD-PS and pco
				...
				f1exp_cam=gui_NameSpace.gui_retr('f1exp_cam');
			end
			if value == 5 %chronos
				...
				cameraIP=gui_NameSpace.gui_retr('Chronos_IP');
				acquisition_NameSpace.acquisition_control_simple_sync_serial(0,0) %stop triggering when already running.
				[OutputError] = PIVlab_capture_chronos_synced_start(cameraIP,cam_fps); %prepare cam and start camera (waiting for trigger...)
				acquisition_NameSpace.acquisition_control_simple_sync_serial(1,0); gui_NameSpace.gui_put('laser_running',1); %turn on laser
			...
			elseif value == 8  %OPTOcam
				...
				min_allowed_interframe = gui_NameSpace.gui_retr('min_allowed_interframe');
				pulse_sep=str2double(get(handles.ac_interpuls,'String'));
			...
			elseif value == 9  %OPTRONIS
				OPTRONIS_bits =gui_NameSpace.gui_retr('OPTRONIS_bits');
			...
			if (~isempty(gui_NameSpace.gui_retr('ac_enable_seeding1')) && gui_NameSpace.gui_retr('ac_enable_seeding1') ~=0) || (~isempty(gui_NameSpace.gui_retr('ac_enable_device1')) && gui_NameSpace.gui_retr('ac_enable_device1') ~=0) || (~isempty(gui_NameSpace.gui_retr('ac_enable_device2')) && gui_NameSpace.gui_retr('ac_enable_device2') ~=0) || (~isempty(gui_NameSpace.gui_retr('ac_enable_flowlab')) && gui_NameSpace.gui_retr('ac_enable_flowlab') ~=0)
				acquisition_NameSpace.acquisition_external_device_control(0); % stops all external devices
			end
			acquisition_NameSpace.acquisition_control_simple_sync_serial(0,0);pause(0.1);acquisition_NameSpace.acquisition_control_simple_sync_serial(0,0);
			...
			if value == 8 %OPTOcam
				if ~isinf(imageamount) % when the nr. of images is inf, then dont save images. nr of images becomes inf when user selects to not save the images.
					if gui_NameSpace.gui_retr('cancel_capture')==1
						answer = questdlg('Save the PIV images that were recorded?', 'Save images?', 'Yes','No','Yes');
						if strcmp(answer , 'Yes')
							gui_NameSpace.gui_put('cancel_capture',0); %user pressed cancel, but still wants to save the recorded images.
			...
			if gui_NameSpace.gui_retr('cancel_capture')==0
				camera_type=gui_NameSpace.gui_retr('camera_type');
				found_the_data=acquisition_NameSpace.acquisition_push_recorded_to_GUI(camera_type,imageamount);
				if found_the_data==1
					gui_NameSpace.gui_put('sessionpath',projectpath );
					set(handles.time_inp,'String',num2str(str2num(get(handles.ac_interpuls,'String'))/1000));
					hgui=getappdata(0,'hgui');
					serpo=getappdata(hgui,'serpo');
					export_NameSpace.export_save_session_function (projectpath,'PIVlab_Capture_Session.mat');
					gui_NameSpace.gui_put('serpo',serpo); %Serpo gets inaccessible after savesession. Probably because there are a number of variables cleared to allow saving without crashing.
				else
	...
	acquisition_NameSpace.acquisition_pco_error_msgbox
end
gui_NameSpace.gui_put('capturing',0);
