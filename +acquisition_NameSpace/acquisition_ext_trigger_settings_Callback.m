...
if ~isempty(serpo)
	...
	if get(handles.ac_enable_ext_trigger,'Value')==1 %execute only if checkbox was off before it was clicked.
		...
		selectedtriggerdelay=gui_NameSpace.gui_retr('selectedtriggerdelay');
		if isempty(selectedtriggerdelay)
			selectedtriggerdelay=100;
		end
		selectedtriggerskip=gui_NameSpace.gui_retr('selectedtriggerskip');
		...
		if ~isempty(answer)
			gui_NameSpace.gui_put('selectedtriggerdelay',str2double(answer{1}));
			gui_NameSpace.gui_put('selectedtriggerskip',str2double(answer{2}));
		end
	end
end
