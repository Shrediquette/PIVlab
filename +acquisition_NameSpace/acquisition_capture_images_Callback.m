...
gui_NameSpace.gui_switchui('multip24')
acquisition_NameSpace.acquisition_select_capture_config_Callback
...
if isempty(get(handles.ac_project,'String')) %if user hasnt entered a project path...
	if ~isempty(gui_NameSpace.gui_retr('pathname'))
		set(handles.ac_project,'String',fullfile(gui_NameSpace.gui_retr('pathname'),['PIVlabCapture_' char(datetime('today'))]));
	else
...
gui_NameSpace.gui_put('ac_upper_clim',2^16);
...
if gui_NameSpace.gui_retr('parallel')==1
	...
	if strncmp(button,'OK',3)==1
		gui_NameSpace.gui_put('parallel',1); %sets to "parallel on" and then presses the toggle button --> will turn off.
		misc_NameSpace.misc_toggle_parallel_Callback
	end
end
