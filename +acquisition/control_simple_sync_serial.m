...
if alreadyconnected
	...
	master_freq =gui.gui_retr('master_freq'); %will depend on the laser system (frequency with best beam quality)
	%frame 1 exposure time incl. readout time in µs
	f1exp = gui.gui_retr('f1exp'); % will depend on camera model
	...
	if get(handles.ac_enable_ext_trigger,'Value') == 0
		...
		extdly = gui.gui_retr('selectedtriggerdelay'); % external trigger input delay. -1 disables external trigger
		extskp = gui.gui_retr('selectedtriggerskip'); %external trigger amount of signals to skip.
	end
	%Camera fps
	...
	laser_device_id=gui.gui_retr('laser_device_id');
	...
	pause(0.1)
	warning off
	serial_answer = acquisition.acquisition_process_sync_reply(serpo);
