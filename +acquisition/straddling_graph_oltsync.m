...
if strcmp(camera_principle,'double_shutter') %double shutter camera triggers second frame internally. This isn't included in the timing_table, so wee add this behaviour here
    ...
    %pco only reacts to rising edge of camera trigger signal. Therefore the camera trigger has a fixed igh time (because it doesn't matter)
    % The camera exposure is controlled via the exposure time setting sent to the camera
    % This means that visualization of falling edge of exposure 1 can't be correct.
    % So I need to use the end of lasre pulse 1 to visualize the end of the exposure
    timing_table{1,2}=timing_table{2,2} % put end of camera exposure 1 to end of pulse 1
    timing_table{1,3}=timing_table{1,2}+1;
    timing_table{1,4}=timing_table{1,3}+1000; %actually the length of the second exposure depends on the read out time of the frames. This is something only the camera knows, so we guess a time. It is for illustration only anyway.
end
x_var=[];
y_var=[];
for i=0:num_reps-1
