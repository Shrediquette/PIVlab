...
if required_files_check
	...
	if strmatch(button,'Yes')==1
		...
		if get(handles.ac_pivcapture_save,'Value')==1 %check settings only when user wants to save data
			...
			capture_ok=acquisition.acquisition_check_project_path(projectpath,'double_images');
		else
		...
		ac_ROI_realtime=gui.gui_retr('ac_ROI_realtime');
		do_realtime=gui.gui_retr('do_realtime');
		...
		if capture_ok==1
			...
			if isinf(imageamount)
				gui.gui_toolsavailable(0,'Starting PIV preview...')
			else
				gui.gui_toolsavailable(0,'Starting PIV capture...')
			end
			...
			if (~isempty(gui.gui_retr('ac_enable_seeding1')) && gui.gui_retr('ac_enable_seeding1') ~=0) || (~isempty(gui.gui_retr('ac_enable_device1')) && gui.gui_retr('ac_enable_device1') ~=0) || (~isempty(gui.gui_retr('ac_enable_device2')) && gui.gui_retr('ac_enable_device2') ~=0) || (~isempty(gui.gui_retr('ac_enable_flowlab')) && gui.gui_retr('ac_enable_flowlab') ~=0)
				acquisition.acquisition_external_device_control(1); %starts activated devices
				waitbar(.15,f,'Starting external devices...');
				pause(0.5)
				waitbar(.33,f,'Starting external devices...');
				pause(0.5)
				if (~isempty(gui.gui_retr('ac_enable_flowlab')) && gui.gui_retr('ac_enable_flowlab') ~=0) %flowlab is activated
					...
					if flowlab_percent ~= 0
						acquisition.acquisition_external_device_control(1);
			...
			if value== 3 || value == 4 %setup with LD-PS and pco
				...
				f1exp_cam=gui.gui_retr('f1exp_cam');
			end
			if value == 5 %chronos
				...
				acquisition.acquisition_control_simple_sync_serial(0,0) %stop triggering when already running.
				[OutputError] = PIVlab_capture_chronos_synced_start(cameraIP,cam_fps); %prepare cam and start camera (waiting for trigger...)
				acquisition.acquisition_control_simple_sync_serial(1,0); gui.gui_put('laser_running',1); %turn on laser
			...
			elseif value == 8  %OPTOcam
				...
				min_allowed_interframe = gui.gui_retr('min_allowed_interframe');
				pulse_sep=str2double(get(handles.ac_interpuls,'String'));
			...
			elseif value == 9  %OPTRONIS
				OPTRONIS_bits =gui.gui_retr('OPTRONIS_bits');
			...
			if (~isempty(gui.gui_retr('ac_enable_seeding1')) && gui.gui_retr('ac_enable_seeding1') ~=0) || (~isempty(gui.gui_retr('ac_enable_device1')) && gui.gui_retr('ac_enable_device1') ~=0) || (~isempty(gui.gui_retr('ac_enable_device2')) && gui.gui_retr('ac_enable_device2') ~=0) || (~isempty(gui.gui_retr('ac_enable_flowlab')) && gui.gui_retr('ac_enable_flowlab') ~=0)
				acquisition.acquisition_external_device_control(0); % stops all external devices
			end
			acquisition.acquisition_control_simple_sync_serial(0,0);pause(0.1);acquisition.acquisition_control_simple_sync_serial(0,0);
			...
			if value == 8 %OPTOcam
				if ~isinf(imageamount) % when the nr. of images is inf, then dont save images. nr of images becomes inf when user selects to not save the images.
					if gui.gui_retr('cancel_capture')==1
						answer = questdlg('Save the PIV images that were recorded?', 'Save images?', 'Yes','No','Yes');
						if strcmp(answer , 'Yes')
							gui.gui_put('cancel_capture',0); %user pressed cancel, but still wants to save the recorded images.
			...
			if gui.gui_retr('cancel_capture')==0
				camera_type=gui.gui_retr('camera_type');
				found_the_data=acquisition.acquisition_push_recorded_to_GUI(camera_type,imageamount);
				if found_the_data==1
					gui.gui_put('sessionpath',projectpath );
					set(handles.time_inp,'String',num2str(str2num(get(handles.ac_interpuls,'String'))/1000));
					hgui=getappdata(0,'hgui');
					serpo=getappdata(hgui,'serpo');
					export.export_save_session_function (projectpath,'PIVlab_Capture_Session.mat');
					gui.gui_put('serpo',serpo); %Serpo gets inaccessible after savesession. Probably because there are a number of variables cleared to allow saving without crashing.
				else
					gui.gui_displogo
				end
			end
