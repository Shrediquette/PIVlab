...
if size(filepath,1) > 1 %did the user load images?
	...
	gui_NameSpace.gui_put('masks_in_frame',[]);%remove existing masks before calculating new ones.
	...
	for i=1:2:num_frames_to_process
		[~,piv_image_A]=import_NameSpace.import_get_img(i);
		[~,piv_image_B]=import_NameSpace.import_get_img(i+1);
		...
		masks_in_frame=mask_NameSpace.mask_px_to_rois(blocations,(i+1)/2,masks_in_frame);
