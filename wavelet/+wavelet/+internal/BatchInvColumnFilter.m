function y = BatchInvColumnFilter(x,ha,hb)
...
% Filter length is even for q-shift filters but not for biorthogonal
Nh2 = Nh/2;
% Allocate array for output
y = coder.nullcopy(zeros(2*Nr,prod([Nc Nchan NIm]),'like',x));
...
s = 1:4:(Nr*2);
% Reshape for conv2
%y = reshape(y,[2*Nr prod([Nc Nchan NIm])]);
if signalwavelet.internal.iseven(Nh2)
        
    t = 4:2:(Nr+Nh);
    % For a lowpass filter, we could do this with a flag
    if dot(ha,hb) > 0
        ta = t; 
        tb = t - 1;
    else
