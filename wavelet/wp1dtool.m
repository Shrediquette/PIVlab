...
switch option
    case 'create'
        ...
        uimenu(m_load,...
            ...
            'Position',1,      ...
            'Callback',@(~,~)wp1dmngr('load_sig', win_wptool) ...
            );
        uimenu(m_load,...
            'Label',getWavMSG('Wavelet:commongui:Str_Decomp'), ...
            'Position',2, ...
            'Callback',@(~,~)wp1dmngr('load_dec', win_wptool) ...
            );

        uimenu(m_save,...
            'Label',getWavMSG('Wavelet:commongui:Str_SynSig'),...
            'Position',1,        ...
            'Callback',@(~,~)wp1dmngr('save_synt', win_wptool) ...
            );
        uimenu(m_save,...
            'Label',getWavMSG('Wavelet:commongui:Str_Decomp'), ...
            'Position',2,         ...
            'Callback',@(~,~)wp1dmngr('save_dec', win_wptool) ...
            );
        ...
        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'sumsin', 'db1', 2, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_2');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'freqbrk', 'haar', 3, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_3');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'mfrqbrk', 'haar', 4, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_4');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'freqbrk', 'haar', 3, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_5');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'vonkoch', 'haar', 3, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_6');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'sinper8', 'db1', 7, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_7');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'qdchirp', 'db1', 3, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_8');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'mishmash', 'db1', 3, 'shannon'));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_9');

        uimenu(m_loadtst,'Separator','on','Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'noisbloc', 'haar', 3, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_10');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'noisbump', 'haar', 2, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_11');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'heavysin', 'haar', 2, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_12');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'noisdopp', 'haar', 2, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_13');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'noischir', 'haar', 2, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_14');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'noismima', 'haar', 2, 'threshold', 0.2));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_15');

        uimenu(m_loadtst,'Separator','on','Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'linchirp', 'db2', 4, 'threshold', 4));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_16');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'quachirp', 'db3', 4, 'threshold', 4));

        lab = getWavMSG('Wavelet:wp1d2dRF:WP1D_Ex_17');

        uimenu(m_loadtst,'Label',lab,'Callback',...
            @(~,~)wp1dmngr('demo', win_wptool,'sumlichr', 'sym3', 4, 'threshold', 4));

        uimenu(m_imp_wrks,...
            'Label',getWavMSG('Wavelet:commongui:Str_ImportSig'),...
            'Tag','Import_Sig', ...
            'Callback',@(~,~)wp1dmngr('import_sig', win_wptool ) ...
            );
        uimenu(m_imp_wrks,...
            'Label',getWavMSG('Wavelet:commongui:Str_ImportDec'), ...
            'Tag','Import_Dec', ...
            'Callback',@(~,~)wp1dmngr('import_dec', win_wptool ) ...
            );

        uimenu(m_exp_wrks,...
            'Tag','Export_Sig', ...
            'Label',getWavMSG('Wavelet:commongui:Str_ExportSig'), ...
            'Callback', @(~,~)wp1dmngr('exp_wrks', win_wptool, 'sig') ...
            );
        uimenu(m_exp_wrks,...
            'Label',getWavMSG('Wavelet:commongui:Str_ExportDec'), ...
            'Tag','Export_Dec', ...
            'Callback', @(~,~)wp1dmngr('exp_wrks', win_wptool, 'dec') ...
            );
        ...
        %------------------------------
        cba_anal       = @(~,~)wp1dmngr('anal', win_wptool);
        cba_comp       = @(~,~)wp1dmngr('comp', win_wptool);
        cba_deno       = @(~,~)wp1dmngr('deno', win_wptool);
        ...
        cba_colm     = @(~,~)wptreeop('col_mode', win_wptool , ...
                        (pop_colm));
    ...
    case 'close'
        ...
        delete(called_win);
        ssig_file = ['ssig_rec.' sprintf('%.0f',double(fig))];
