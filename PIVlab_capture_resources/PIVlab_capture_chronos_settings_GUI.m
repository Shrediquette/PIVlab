...
if isempty(fh)
	...
	chronos_control_window = figure('numbertitle','off','MenuBar','none','DockControls','off','Name','Chronos settings','Toolbar','none','Units','characters','Position', [mainpos(1)+mainpos(3)-35 mainpos(2)+15+4+4 35 10+1.5],'tag','chronos_control_window','visible','on','KeyPressFcn', @key_press,'resize','off');
	...
	handles.ip_input = uicontrol(handles.mainpanel,'Style','edit','String','192.168.162.20','Units','characters', 'Fontunits','points','Position',[item(1)+margin parentitem(4)-item(4)-margin-item(2) item(3)-margin*2 item(4)],'Callback', {@device_set,1,'off'} ,'tag','ip_input');
	...
	item=[parentitem(3)/2*0 item(2)+item(4)+0.25 parentitem(3)/2 2];
...
pause(1)
response = webwrite([cameraURL '/control/reboot'],struct('power',1,'reload',1,'settings',1),options);
