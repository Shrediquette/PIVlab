...
gui_NameSpace.gui_put('derived',[]) %calibration makes previously derived params incorrect
...
if numel(pointscali)>0
	...
	gui_NameSpace.gui_put('calu',calu);
	gui_NameSpace.gui_put('calv',calv);
	gui_NameSpace.gui_put('calxy',calxy);
	...
	if numel(points_offsetx)>0
		offsetx = calibrate_NameSpace.calibrate_calculate_offset_axis('x',points_offsetx(1),points_offsetx(3));
		gui_NameSpace.gui_put('offset_x_true',offsetx);
	...
	if numel(points_offsety)>0
		offsety = calibrate_NameSpace.calibrate_calculate_offset_axis('y',points_offsety(2),points_offsety(3));
		gui_NameSpace.gui_put('offset_y_true',offsety);
	else %no offsets applied
