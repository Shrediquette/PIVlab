function [v1,output,alpha]=wOFVBayesPyd_sub(I0Org,I1Org,C,LOrg,eta,varthetaOrg,v0,regscheme,maskOrg,Fmats,PyramidLevels,MedianFilter,FiltImg)
...
ImPy0 = GenerateImPyramid(I0Org,PyramidLevels,FiltImg);
ImPy1 = GenerateImPyramid(I1Org,PyramidLevels,FiltImg);
MaskPy = GenerateImPyramid(maskOrg,PyramidLevels,FiltImg);
VarThetaPy = GenerateImPyramid(varthetaOrg,PyramidLevels,FiltImg);
...
%     disp(['Pyramid Level: ' num2str(Level) ' out of ' num2str(PyramidLevels)])
    ...
    Theta=cat(3,Psitrunc_mat(v0(:,:,1),Fw(1:2^C,:)),Psitrunc_mat(v0(:,:,2),...
        Fw(1:2^C,:)));
    ...
    for scale=C:L
        ...
        call_wOFVBayes(Theta,I0int,I1int,scale,eta,vartheta,Fw(1:2^scale,:),FwInv(:,...
        ...
        [Theta,~,~,output]=minFunc(@call_wOFVBayes,Theta,minOptions);
    ...
    if 2^L<n
        v1=cat(3,Psitrunc_mat(Theta(:,:,1),FwInv(:,1:2^(L+1))).*mask,...
            Psitrunc_mat(Theta(:,:,2),FwInv(:,1:2^(L+1))).*mask);
    else
        Theta=Theta(1:n,1:n,:);

        v1=cat(3,Psitrunc_mat(Theta(:,:,1),FwInv).*mask,Psitrunc_mat(Theta(...
            :,:,2),FwInv).*mask);
    end
...
alpha = getalphafield(I0int,I1int,v1,mask,eta,vartheta,Grid);
