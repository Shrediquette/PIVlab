...
if ok==1
	...
	bg_img_A = gui_NameSpace.gui_retr('bg_img_A'); %contains bg image, or is empty array
	...
	[tmp,~]=import_NameSpace.import_get_img(1);
	...
	for ii=1:floor(size(filepath,1)/2)
		...
		converted_mask{ii}=mask_NameSpace.mask_convert_masks_to_binary(size(tmp(:,:,1)),mask_positions);
	end
