...
if ok==1
	...
	if numel(roirect) == 4
		...
		if roirect(3) < 50 || roirect(4)< 50
			...
			drawnow
			[A,~] = import_NameSpace.import_get_img(selected);
			[B,~] = import_NameSpace.import_get_img(selected+1);
			...
			piv_NameSpace.piv_pass2_checkbox_Callback(handles.checkbox26)
			piv_NameSpace.piv_pass3_checkbox_Callback(handles.checkbox27)
			piv_NameSpace.piv_pass4_checkbox_Callback(handles.checkbox28)
			piv_NameSpace.piv_pass2_size_Callback(handles.edit50)
			piv_NameSpace.piv_pass3_size_Callback(handles.edit51)
			piv_NameSpace.piv_pass4_size_Callback(handles.edit52)
			piv_NameSpace.piv_fftmulti_Callback(handles.fftmulti)
			piv_NameSpace.piv_step_Callback(handles.step)
