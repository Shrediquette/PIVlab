...
if get(handles.bg_subtract,'Value')==1
	...
	sequencer=gui_NameSpace.gui_retr('sequencer');%Timeresolved or pairwise 0=timeres.; 1=pairwise
	if sequencer ~= 2 % bg subtraction only makes sense with time-resolved and pairwise sequencing style, not with reference style.
		if isempty(bg_img_A) || isempty(bg_img_B)
			...
			if strcmp(answer , 'OK')
				...
				gui_NameSpace.gui_put('bg_img_A',image1_bg);
				if sequencer==1 %not time-resolved
					gui_NameSpace.gui_put('bg_img_B',image2_bg);
				else
					gui_NameSpace.gui_put('bg_img_B',image1_bg); %timeresolved --> same bg image for a and b
				end
