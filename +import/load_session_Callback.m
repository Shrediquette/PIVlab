...
if auto_load_session ~= 1
    ...
    if ~isequal(FileName,0) & ~isequal(PathName,0)
        load(fullfile(PathName,FileName),'resultslist','wasdisabled','sequencer')
        if exist('resultslist','var') && exist('wasdisabled','var') && exist('sequencer','var')
            valid_session_file=1;
        else
...
elseif valid_session_file == 1
    ...
    try
        ...
        vars=load(fullfile(PathName,FileName),'yposition', 'FileName', 'PathName', 'add_header', 'addfileinfo', 'autoscale_vec', 'caliimg', 'calu', 'calv','calxy', 'cancel', 'clahe_enable', 'clahe_size', 'colormap_choice', 'colormap_steps', 'colormap_interpolation', 'delimiter', 'derived', 'displaywhat', 'distance', 'enable_highpass', 'enable_intenscap', 'epsilon', 'filename', 'filepath', 'framenum','framepart', 'highp_size', 'homedir', 'img_not_mask', 'intarea', 'interpol_missing', 'loc_med_thresh', 'loc_median', 'manualdeletion', 'pathname', 'pointscali', 'resultslist', 'roirect', 'sequencer', 'sessionpath', 'stdev_check', 'stdev_thresh', 'stepsize', 'subpix', 'subtr_u', 'subtr_v', 'toggler', 'vectorscale', 'velrect', 'wasdisabled', 'xposition','realdist_string','time_inp_string','streamlinesX','streamlinesY','manmarkersX','manmarkersY','algorithm_selection','pass2','pass3','pass4','pass2val','pass3val','pass4val','step2','step3','step4','holdstream','streamlamount','streamlcolor','ismean','wienerwurst','wienerwurstsize','mask_auto_box','Autolimit','minintens','maxintens','CorrQuality_nr','enhance_disp','video_selection_done','video_frame_selection','video_reader_object','bg_img_A','bg_img_B','x_axis_direction','y_axis_direction','size_of_the_image','points_offsetx','points_offsety','offset_x_true','offset_y_true','bright_filter_thresh','contrast_filter_thresh','do_bright_filter','do_contrast_filter','repeat_last','repeat_last_thresh','do_corr2_filter','corr_filter_thresh','notch_L_thresh','notch_H_thresh','notch_filter','masks_in_frame','pcopanda_dbl_image','velrect_freehand','calib_boardtype','calib_origincolor','calib_rows','calib_columns','calib_checkersize','calib_markersize','calib_dolivedetect','calib_viewtype','calib_usecalibration','calib_userectification','cam_selected_rectification_image','cam_selected_target_images','cam_use_calibration','cam_use_rectification','cameraParams','rectification_tform','expected_image_size','calib_upscale', 'stereomode','calib_use_tilted_model','cam_use_tilted_model','cam_tilted_D','cam_K_opencv','bg_mode','valid_color_idx','secondpeak_color_idx','interp_color_idx','deriv_color_idx','extrapolate_border','smooth_mode_val','smooth_param_str','temporal_window_str');
    catch
    ...
    try
        if isfield(vars,'valid_color_idx')
            set(handles.valid_color,      'Value', vars.valid_color_idx);
            set(handles.secondpeak_color, 'Value', vars.secondpeak_color_idx);
            set(handles.interp_color,     'Value', vars.interp_color_idx);
            set(handles.deriv_color,      'Value', vars.deriv_color_idx);
        end
    catch
    end
    ...
    try
        set(handles.extrapolate_border,'Value',vars.extrapolate_border);
    catch
    end
    ...
    try
        if vars.velrect_freehand(1,1)~=0 && vars.velrect_freehand(1,2)~=0
            gui.put('velrect_freehand', vars.velrect_freehand);
            validate.update_velocity_limits_information
        end
    catch
    end
    ...
    try
        if ~isempty(vars.bg_img_A)
			set(handles.bg_subtract,'Value',vars.bg_mode);
        else
    ...
    try
        if isfield(vars,'smooth_mode_val')
            set(handles.smooth_mode,'value',vars.smooth_mode_val);
        end
        if isfield(vars,'smooth_param_str')
            set(handles.smooth_param,'string',vars.smooth_param_str);
        end
        if isfield(vars,'temporal_window_str')
            set(handles.temporal_window,'string',vars.temporal_window_str);
        end
        ...
        disp('couldnt set data smoothing GUI elements for load_session')
    end
    %reset zoom
